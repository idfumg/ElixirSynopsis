defmodule Pooly.PoolServer do
  ...
  defmodule State do
    defstruct sup: nil, size: nil, mfa: nil, worker_sup: nil, workers: nil, monitors: nil, name: nil, max_overflow: nil, overflow: nil
  end
  ...
  def init([sup, pool_config]) when is_pid(sup) do
    ...
    init(pool_config, %State{sup: sup, monitors: monitors, overflow: 0})
  end
  ...
  def handle_info(:start_worker_supervisor, state = %{sup: sup, mfa: mfa, size: size, name: name}) do
    {:ok, worker_sup} = Supervisor.start_child(sup, supervisor_spec(name, mfa))
  ...
  defp supervisor_spec(name, mfa) do
    opts = [id: name <> "WorkerSupervisor", restart: :temporary, name: name <> "WorkerSupervisor"]
  ...
  defp handle_checkin(pid, state) do
    ...
    if overflow > 0 do
      :ok = dissmiss_worker(worker_sup, pid)
      %{state | overflow: overflow - 1}
    else
      {:noreply, %{state | workers: [pid | workers], overflow: 0}}
    end
  end

  defp dissmiss_worker(worker_sup, pid) do
