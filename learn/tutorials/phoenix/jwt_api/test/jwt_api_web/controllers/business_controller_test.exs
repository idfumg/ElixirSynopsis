defmodule JwtApiWeb.BusinessControllerTest do
  use JwtApiWeb.ConnCase

  alias JwtApi.Directory
  ...
  describe "create business" do
    test "renders business when data is valid", %{conn: conn} do
      ...
      assert json_response(conn, 200)["data"] == %{
        ...
        "name" => "some name",
        "tag" => "some tag",
        "inserted_at" => NaiveDateTime.to_string(Directory.get_business!(id).inserted_at),
        "updated_at" => NaiveDateTime.to_string(Directory.get_business!(id).updated_at),
      }
    end
